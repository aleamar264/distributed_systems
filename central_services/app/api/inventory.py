...
@router.post("/update")
async def update_inventory(payload: UpdateInventory, db: AsyncSession = Depends(get_db)):
    async with db.begin():
        ...
        update_values = {"quantity": new_quantity, "version":item.version, "updated_at": datetime.now(UTC)}
        await update_inventory_services(db=db, update_values=update_values, sku=payload.sku)
    return {"sku": payload.sku, "quantity": new_quantity, "version": payload.version}
